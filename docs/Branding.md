...
VITE_AGENTHUB_CONSOLE_ICON_URL="/brand/agenthub-console.svg"
```
...
- [agenthub-console-app.yaml.tmpl](../deploy/manifests/agenthub-console-app.yaml.tmpl)

对应字段：

```yaml
# agenthub-app.yaml.tmpl
spec:
  icon: "{{ .iconUrl }}" # Agent Hub 主入口

# agenthub-console-app.yaml.tmpl
spec:
  icon: "{{ .agentHubUrl }}/brand/agenthub-console.svg" # Agent Hub Console hidden app
```
...
- 隐式 App：
  - `agenthub-console`
  - `displayType: hidden`
  - 只供 `openDesktopApp` 打开控制台窗口，不直接暴露给用户

这意味着：

- 如果是部署 Sealos App 图标，需要给模板渲染参数传一个 SVG 的公网 URL
- Agent Hub 主入口继续使用 `.iconUrl`
- Agent Hub Console hidden app 使用前端静态资源 `/brand/agenthub-console.svg`
- 不能直接传本地文件系统路径
