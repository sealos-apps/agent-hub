...
export const parseAgentTerminalDesktopMessage = (raw: unknown) => {
  ...
  if (direct.type === AGENTHUB_CONSOLE_MESSAGE_TYPE && directAgentName) {
    return directAgentName
  }

  const routeAgentName = readAgentName(direct.query?.agentName)
  if (direct.pathname === AGENTHUB_CONSOLE_ROUTE && routeAgentName) {
  ...
  if (nestedMessageData?.type === AGENTHUB_CONSOLE_MESSAGE_TYPE && nestedAgentName) {
  ...
  if (eventBusPayload?.type === AGENTHUB_CONSOLE_MESSAGE_TYPE && eventBusAgentName) {
    return eventBusAgentName
  }

  const eventBusRouteAgentName = readAgentName(eventBusPayload?.query?.agentName)
  if (eventBusPayload?.pathname === AGENTHUB_CONSOLE_ROUTE && eventBusRouteAgentName) {
